...
const EditProperty = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // ✅ إضافة الصور الجديدة فقط (Files)
      const newImages = images.filter((img) => img instanceof File);
      newImages.forEach((image, index) => {
        formData.append('images', image);
        console.log(`✅ Added new image ${index + 1}:`, image.name);
      });
      ...
      const response = await propertyService.update(id, formData);
      ...
      if (err.response?.data?.errors) {
        ...
        setError(err.response?.data?.message || 'حدث خطأ أثناء تحديث العقار');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-bold text-gray-900">تعديل العقار</h1>
      </div>
